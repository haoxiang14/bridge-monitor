...
async function checkOneAdapter(cfg: AdapterConfig): Promise<OftAdapterResult | null> {
  ...
  console.log(`[OFT ADAPTER] ${cfg.symbol} (${cfg.chain}): locked=${locked.toFixed(2)} minted=${totalMinted.toFixed(2)} delta=${delta?.toFixed(2)} status=${status}`);
...
async function checkOneOftToken(cfg: OftTokenConfig): Promise<OftTokenResult | null> {
  ...
  if (successCount > 0) {
    console.log(`[OFT TOKEN] ${cfg.symbol}: totalSupply=${totalSupply.toFixed(2)} chains=${successCount}/${cfg.deployments.length}`);
  }
