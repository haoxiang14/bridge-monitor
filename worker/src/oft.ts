...
// Static adapter config from CSV (adapter_holder entries)
// Each entry: source chain, symbol, token address (locked asset), adapter contract address
interface CsvAdapter {
  chain: string; // CSV chain name
  symbol: string;
  tokenAddress: string;
...
// Static OFT token config from CSV (token_is_oft entries, one per unique token)
// These are pure OFTs — we look up all chain deployments from LZ API
interface CsvOftToken {
  symbol: string;
  lookupAddress: string; // one known address to find in LZ API
  lookupChain: string; // CSV chain for that address
}
...
// Build reverse index: lowercase adapter/OFT address → { symbol, entry, chainKey }
interface LzMatch {
  symbol: string;
  entry: LzTokenEntry;
}
...
export async function checkOftAdapters(): Promise<OftAdapterResult[]> {
  ...
  for (const csv of CSV_ADAPTERS) {
    ...
    if (!rpcKey) {
      console.log(`[OFT ADAPTER] ${csv.symbol} (${csv.chain}): skipped (no RPC for chain)`);
      continue;
    }
    ...
    // Get locked amount: balanceOf(tokenAddress) held by adapter
    ...
    if (locked === null || locked === 0) {
      console.log(`[OFT ADAPTER] ${csv.symbol} (${csv.chain}): skipped (locked=${locked})`);
      continue;
    }

    // Find destination chains from LZ API
    ...
    console.log(`[OFT ADAPTER] ${csv.symbol} (${csv.chain}): locked=${locked.toFixed(2)} minted=${totalMinted.toFixed(2)} delta=${delta?.toFixed(2)} status=${status}`);
  }
...
export async function checkOftTokens(): Promise<OftTokenResult[]> {
  ...
  for (const csv of CSV_OFT_TOKENS) {
    ...
    if (!match) {
      console.log(`[OFT TOKEN] ${csv.symbol}: not found in LayerZero API (address: ${csv.lookupAddress})`);
      continue;
    }
    ...
    if (successCount > 0) {
      console.log(`[OFT TOKEN] ${csv.symbol}: totalSupply=${totalSupply.toFixed(2)} chains=${successCount}/${allDeployments.length}`);
    }
  }
