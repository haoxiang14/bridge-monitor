...
const WALLETS_CACHE_TTL = 300_000;
...
async function checkSingleToken(token: XStocksToken, wallets: SystemWalletsCache): Promise<XStocksResult> {
  ...
  const evmResults = await Promise.all(
    evmChainsNonSolana.map(async (chain) => {
      ...
      const balances = await Promise.all(
        chainWallets.map(async (wallet) => getWalletBalance(chain, tokenAddr, wallet))
      );
      const totalSystemHeld = balances.reduce((sum, b) => sum + b, BigInt(0));
  ...
  for (const result of [...evmResults, ...solanaResults, ...tronResults]) {
    if (result.supply !== null) {
      ...
      totalSupplyAll += Number(result.supply) / divisor;
      systemHeldAll += result.systemHeld !== null ? Number(result.systemHeld) / divisor : 0;
      successCount++;
    }
  }
